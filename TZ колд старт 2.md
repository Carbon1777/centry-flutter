...
Формат email группы 1: `cs_user_NNNN@coldstart.centry.internal`
Для группы 2 продолжаем нумерацию: `cs_user_0362@coldstart.centry.internal` и далее (группа 1 = 0001–0361).

```sql
INSERT INTO auth.users (id, email, encrypted_password, raw_user_meta_data, email_confirmed_at, created_at, updated_at)
VALUES (
  gen_random_uuid(),
  'cs_user_0362@coldstart.centry.internal',
...
```
**Важно**: `display_name` совпадает с именем из скобок в названии папки.

#### 4.3 Создать профиль
```sql
INSERT INTO user_profiles (
  user_id, nickname, name, gender, age,
  rest_preferences, rest_dislikes, avatar_kind, avatar_url
) VALUES (
  <app_user_id>, <nickname>, <display_name>, <gender>, <age>,
  ARRAY[<rest_preferences>]::text[], ARRAY[<rest_dislikes>]::text[],
  <avatar_kind>, <avatar_url>
);
```

**avatar_kind и avatar_url:**
- Если есть сгенерированный аватар (avatar.jpg) → `avatar_kind = 'custom'`, `avatar_url = 'https://<supabase_url>/storage/v1/object/public/avatars/custom/<auth_user_id>/avatar.webp'`
- Если системная аватарка или пусто → `avatar_kind = 'system'`, `avatar_url = NULL`

**rest_preferences** — массив из 1–3 значений:
...
**rest_dislikes** — массив из 0–1 значений:
- `rest_dislike_long_walks`, `rest_dislike_large_groups`
- `rest_dislike_spontaneous`, `rest_dislike_late`
- `rest_dislike_strict_plans`

**profile_visibility**: оставить NULL (как у группы 1).

#### 4.4 Загрузить фото

**Два хранилища:**
1. **Аватар** → bucket `avatars`, путь `custom/<auth_user_id>/avatar.webp`
2. **Альбомные фото** → bucket `profile-photos` (или аналогичный), storage_key формат `<app_user_id>/<uuid>.webp`

```sql
-- Для каждого альбомного фото
INSERT INTO profile_photos (
  id, user_id, storage_key, sort_order, status,
  width, height, mime_type, size_bytes
) VALUES (
  gen_random_uuid(), <app_user_id>,
  '<app_user_id>/<gen_random_uuid()>.webp',
  <sort_order>, 'ready',
...
- Аватар **НЕ** хранится в profile_photos — он в отдельном bucket `avatars`
...
**ПРОВЕРЕНО. Группа 1 занимает:**

Москва:
| Район | Группа 1 | Группа 2 | Пересечение? |
|-------|----------|----------|-------------|
| arbat | Team1–3 | Team4–6 | Нет |
| basmanniy | Team1–2 | Team3–4 | Нет |
| hamovniki | Team1–2 | Team3–4 | Нет |
| krasnoselskiy | Team1–2 | Team3–4 | Нет |
| meshchanskiy | Team1–2 | Team3–4 | Нет |
| presnenskiy | Team1–3 | Team4–6 | Нет |
| taganskiy | Team1–4 | Team5–8 | Нет |
| tverskoy | Team1–4 | Team5–8 | Нет |
| yakimanka | Team1 | Team2–3 | Нет |
| zamoskvoreche | Team1–2 | Team3–4 | Нет |

Питер:
| Район | Группа 1 | Группа 2 | Пересечение? |
|-------|----------|----------|-------------|
| admiralteyskiy | Team1–4 | Team5–8 | Нет |
| kalininskiy | Team1–2 | Team3–4 | Нет |
| moskovskiy | Team1–3 | Team4–6 | Нет |
| nevskiy | Team1–2 | Team3–4 | Нет |
| petrogradskiy | Team1–3 | Team4–6 | Нет |
| primorskiy | Team1–2 | Team3–4 | Нет |
| tsentralniy | Team1–4 | Team5–8 | Нет |
| vasileostrovskiy | Team1–2 | Team3–4 | Нет |
| vyborgskiy | Team1–2 | Team3–4 | Нет |

**Пересечений нет** — группа 2 продолжает нумерацию команд в каждом районе.
